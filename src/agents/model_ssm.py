"""
Modelo MarioNet com State Space Model (SSM) ao invés de Transformer.

Este arquivo contém a implementação da rede neural usando SSM para
processamento eficiente de tokens visuais.
"""
...
# Importar a camada SSM
from src.agents.ssm_layer import SSMLayer, EfficientSSMLayer
...
class MarioNetSSM(nn.Module):
    """CNN com arquitetura Mixture of Experts (MoE) usando State Space Model - 16 especialistas, top-4"""
...
class ActorCriticMarioNetSSM(nn.Module):
    """
    Rede Actor-Critic para GPPO com arquitetura Mixture of Experts (MoE) e SSM.
...
if __name__ == "__main__":
    # Teste do modelo SSM
    print("Testando MarioNetSSM...")
    
    # Configuração do modelo
    state_dim = (4, 84, 84)  # 4 frames empilhados, 84x84
    action_dim = 6  # Número de ações possíveis
    ...
    # Testar target network
    output_target = model(x, model="target")
    print(f"Saída target: {output_target.shape}")
    
    # Testar ActorCritic
    print("\nTestando ActorCriticMarioNetSSM...")
    ac_model = ActorCriticMarioNetSSM(state_dim, action_dim)
    ...
    print(f"\nTotal de parâmetros (MarioNetSSM): {total_params:,}")
    
    total_params_ac = sum(p.numel() for p in ac_model.parameters())
    print(f"Total de parâmetros (ActorCriticMarioNetSSM): {total_params_ac:,}")
